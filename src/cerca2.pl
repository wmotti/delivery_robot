...
cerca2(F,U) :-
	...
	vicini2(N,L),              % dal problema
	trasforma2(L,nc(N,P,C),NL),
	aggiungi(NL,F1,NF),       % dalla strategia
        cerca2(NF,U).		  % ricorsione coda
...
/*
trasforma2([],_,[]).
trasforma2([   V               | R ], nc(N,Camm,Cost), 
          [nc(V, [N|Camm], NC)| NR]) 
        :-
        not(member(N, Camm)),!,
        costo2(N,V,CA),            % dal problema
        NC is Cost + CA,
        trasforma2(R, nc(N,Camm,Cost), NR). 
*/

trasforma2([],_,[]).
trasforma2([   V               | R ], nc(N,Camm,Cost), 
          [nc(V, NuovoCammino, NC)| NR]) 
        :-
        %not(member(N, Camm)),!,
        path(N,V,NCamm,_),
	%reverse(NCamm, RevNCamm),
	append(NCamm,Camm,NuovoCammino),
        
        costo2(N,V,CA),            % dal problema
        NC is Cost + CA,
        trasforma2(R, nc(N,Camm,Cost), NR). 

trasforma2([_ | R ], nc(N,Camm,Cost), NR) :-
         trasforma2(R, nc(N,Camm,Cost), NR).